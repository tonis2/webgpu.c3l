...
const TEXTURE_FORMAT = {
    22: 'rgba8unorm', 23: 'rgba8unorm-srgb',
    24: 'rgba8snorm', 25: 'rgba8uint', 26: 'rgba8sint',
    27: 'bgra8unorm', 28: 'bgra8unorm-srgb',
    46: 'depth24plus', 47: 'depth24plus-stencil8',
...
const ADDRESS_MODE = { 0: undefined, 1: 'clamp-to-edge', 2: 'repeat', 3: 'mirror-repeat' };
const FILTER_MODE = { 0: undefined, 1: 'nearest', 2: 'linear' };
const MIPMAP_FILTER_MODE = { 0: undefined, 1: 'nearest', 2: 'linear' };
const SAMPLER_BINDING_TYPE = { 0: undefined, 1: undefined, 2: 'filtering', 3: 'non-filtering', 4: 'comparison' };
const TEXTURE_SAMPLE_TYPE = { 0: undefined, 1: undefined, 2: 'float', 3: 'unfilterable-float', 4: 'depth', 5: 'sint', 6: 'uint' };
const TEXTURE_VIEW_DIM = { 0: undefined, 1: '1d', 2: '2d', 3: '2d-array', 4: 'cube', 5: 'cube-array', 6: '3d' };
...
// Decoded image data (set before WASM init)
...
const OFF = {
    ...
    // +40: SamplerBindingLayout { +40: void* next, +44: type(u32) }
    // +48: TextureBindingLayout { +48: void* next, +52: sample_type(u32), +56: view_dimension(u32), +60: multisampled(u32) }
    BGLEntry: { binding: 4, visibility: 8, buffer_type: 20, buffer_has_dynamic: 24, buffer_min_size: 32,
                sampler_type: 44, texture_sample_type: 52, texture_view_dim: 56, texture_multisampled: 60 },

    // SamplerDescriptor (52 bytes)
    // +0: void* next, +4: StringView label(8), +12: address_mode_u, +16: v, +20: w,
    // +24: mag_filter, +28: min_filter, +32: mipmap_filter,
    // +36: lod_min_clamp(f32), +40: lod_max_clamp(f32), +44: compare, +48: max_anisotropy(u16)
    SampDesc: { address_mode_u: 12, address_mode_v: 16, address_mode_w: 20,
                mag_filter: 24, min_filter: 28, mipmap_filter: 32,
                lod_min_clamp: 36, lod_max_clamp: 40, compare: 44, max_anisotropy: 48 },
    ...
    // TexelCopyTextureInfo (24 bytes)
    // +0: Texture, +4: mip_level, +8: Origin3D {x,y,z} (12), +20: TextureAspect
    TexCopyTex: { texture: 0, mip_level: 4, origin_x: 8, origin_y: 12, origin_z: 16, aspect: 20 },

    // TexelCopyBufferLayout (16 bytes)
    // +0: ulong offset (8), +8: uint bytes_per_row, +12: uint rows_per_image
    TexCopyBufLayout: { offset: 0, bytes_per_row: 8, rows_per_image: 12 },

    // Extent3D (12 bytes)
    Extent3D: { width: 0, height: 4, depth: 8 },
...
export function createBridge() {
    const env = {
        ...
        // (i32 queue, i32 dest_ptr, i32 data_ptr, i32 data_size, i32 layout_ptr, i32 size_ptr) -> void
        wgpuQueueWriteTexture(queueHandle, destPtr, dataPtr, dataSize, layoutPtr, sizePtr) {
            const queue = getHandle(queueHandle);
            const texture = getHandle(readU32(destPtr + OFF.TexCopyTex.texture));
            const mipLevel = readU32(destPtr + OFF.TexCopyTex.mip_level);
            const destination = { texture, mipLevel };
            const bytesPerRow = readU32(layoutPtr + OFF.TexCopyBufLayout.bytes_per_row);
            const rowsPerImage = readU32(layoutPtr + OFF.TexCopyBufLayout.rows_per_image);
            const dataLayout = { bytesPerRow, rowsPerImage };
            const width = readU32(sizePtr + OFF.Extent3D.width);
            const height = readU32(sizePtr + OFF.Extent3D.height);
            const depthOrArrayLayers = readU32(sizePtr + OFF.Extent3D.depth) || 1;
            const data = new Uint8Array(wasmMemory.buffer, dataPtr, dataSize);
            queue.writeTexture(destination, data, dataLayout, { width, height, depthOrArrayLayers });
        },
        ...
        wgpuDeviceCreateBindGroupLayout(deviceHandle, descPtr) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const sampType = readU32(ePtr + OFF.BGLEntry.sampler_type);
                if (sampType && SAMPLER_BINDING_TYPE[sampType]) {
                    entry.sampler = { type: SAMPLER_BINDING_TYPE[sampType] };
                }
                const texSampleType = readU32(ePtr + OFF.BGLEntry.texture_sample_type);
                if (texSampleType && TEXTURE_SAMPLE_TYPE[texSampleType]) {
                    entry.texture = {
                        sampleType: TEXTURE_SAMPLE_TYPE[texSampleType],
                        viewDimension: TEXTURE_VIEW_DIM[readU32(ePtr + OFF.BGLEntry.texture_view_dim)] || '2d',
                        multisampled: !!readU32(ePtr + OFF.BGLEntry.texture_multisampled),
                    };
                }
                entries.push(entry);
            }
        ...
        wgpuDeviceCreateBindGroup(deviceHandle, descPtr) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const sampHandle = readU32(ePtr + OFF.BGEntry.sampler);
                const tvHandle = readU32(ePtr + OFF.BGEntry.texture_view);
                if (bufHandle) {
                    entries.push({
                        binding,
                    ...
                    entries.push({ binding, resource: getHandle(tvHandle) });
                } else if (sampHandle) {
                    entries.push({ binding, resource: getHandle(sampHandle) });
                }
            }
        ...
        // (i32 device, i32 desc_ptr) -> i32 sampler
        wgpuDeviceCreateSampler(deviceHandle, descPtr) {
            const device = getHandle(deviceHandle);
            const desc = {};
            const u = readU32(descPtr + OFF.SampDesc.address_mode_u);
            const v = readU32(descPtr + OFF.SampDesc.address_mode_v);
            const w = readU32(descPtr + OFF.SampDesc.address_mode_w);
            if (ADDRESS_MODE[u]) desc.addressModeU = ADDRESS_MODE[u];
            if (ADDRESS_MODE[v]) desc.addressModeV = ADDRESS_MODE[v];
            if (ADDRESS_MODE[w]) desc.addressModeW = ADDRESS_MODE[w];
            const mag = readU32(descPtr + OFF.SampDesc.mag_filter);
            const min = readU32(descPtr + OFF.SampDesc.min_filter);
            const mip = readU32(descPtr + OFF.SampDesc.mipmap_filter);
            if (FILTER_MODE[mag]) desc.magFilter = FILTER_MODE[mag];
            if (FILTER_MODE[min]) desc.minFilter = FILTER_MODE[min];
            if (MIPMAP_FILTER_MODE[mip]) desc.mipmapFilter = MIPMAP_FILTER_MODE[mip];
            const lodMin = dv().getFloat32(descPtr + OFF.SampDesc.lod_min_clamp, true);
            const lodMax = dv().getFloat32(descPtr + OFF.SampDesc.lod_max_clamp, true);
            if (lodMin !== 0) desc.lodMinClamp = lodMin;
            if (lodMax !== 0) desc.lodMaxClamp = lodMax;
            const cmp = readU32(descPtr + OFF.SampDesc.compare);
            if (COMPARE_FUNCTION[cmp]) desc.compare = COMPARE_FUNCTION[cmp];
            return addHandle(device.createSampler(desc));
        },
        ...
        // Image bridge functions (decoded images set via setDecodedImages)
        js_get_image_count() {
            return decodedImages ? decodedImages.length : 0;
        },

        js_get_image_info(index, outPtr) {
            const img = decodedImages ? decodedImages[index] : null;
            if (!img) { writeU32(outPtr, 0); writeU32(outPtr + 4, 0); return; }
            writeU32(outPtr, img.width);
            writeU32(outPtr + 4, img.height);
        },

        // Destroy methods — call GPU .destroy() and remove from handle table
        wgpuBufferDestroy(h) { const o = getHandle(h); if (o) { o.destroy(); handles.delete(h); } },
        wgpuTextureDestroy(h) { const o = getHandle(h); if (o) { o.destroy(); handles.delete(h); } },
        wgpuQuerySetDestroy(h) { const o = getHandle(h); if (o) { o.destroy(); handles.delete(h); } },

        // Release methods — remove from handle table (browser GCs the GPU object)
        wgpuBindGroupRelease(h) { handles.delete(h); },
        wgpuBindGroupLayoutRelease(h) { handles.delete(h); },
        wgpuPipelineLayoutRelease(h) { handles.delete(h); },
        wgpuRenderPipelineRelease(h) { handles.delete(h); },
        wgpuShaderModuleRelease(h) { handles.delete(h); },
        wgpuSamplerRelease(h) { handles.delete(h); },
        wgpuTextureViewRelease(h) { handles.delete(h); },
        wgpuCommandEncoderRelease(h) { handles.delete(h); },
        wgpuCommandBufferRelease(h) { handles.delete(h); },
        wgpuRenderPassEncoderRelease(h) { handles.delete(h); },

        js_copy_image_to_texture(index, textureHandle) {
            const img = decodedImages ? decodedImages[index] : null;
            if (!img) return;
            const texture = getHandle(textureHandle);
            preQueue.writeTexture(
                { texture },
                img.data,
    ...
    return {
        ...
        setDecodedImages(images) { decodedImages = images; },
    };
}
